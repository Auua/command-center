...
describe('BraindumpListResponseSchema', () => {
  it('accepts empty and populated lists', () => {
    ...
    expect(BraindumpListResponseSchema.parse({ items: [NOTE] }).items).toHaveLength(1);
  });
});
...
describe.each([
  ...
  it('rejects content above the 20k cap', () => {
    expect(schema.safeParse({ content: 'x'.repeat(20_001) }).success).toBe(false);
    expect(schema.safeParse({ content: 'x'.repeat(20_000) }).success).toBe(true);
  });
