...
describe("TaskListResponseSchema", () => {
  it("accepts empty and populated lists", () => {
    ...
    expect(TaskListResponseSchema.parse({ items: [TASK] }).items).toHaveLength(
      1,
    );
  });
});
...
describe("CreateTaskRequestSchema", () => {
  ...
  it("trims the title and rejects empty/whitespace-only titles", () => {
    expect(CreateTaskRequestSchema.parse({ title: "  pay rent  " }).title).toBe(
      "pay rent",
    );
  ...
  it("rejects titles above the 500-char cap", () => {
    expect(
      CreateTaskRequestSchema.safeParse({ title: "x".repeat(501) }).success,
    ).toBe(false);
    expect(
      CreateTaskRequestSchema.safeParse({ title: "x".repeat(500) }).success,
    ).toBe(true);
  });

  it("dedupes tags and rejects empty tags", () => {
    expect(
      CreateTaskRequestSchema.parse({ title: "t", tags: ["a", "a", "b"] })
        .tags,
    ).toEqual(["a", "b"]);
    expect(
      CreateTaskRequestSchema.safeParse({ title: "t", tags: [" "] }).success,
    ).toBe(false);
  });
});
...
describe("UpdateTaskRequestSchema", () => {
  ...
  it("rejects unknown top-level fields (write paths are strict)", () => {
    expect(
      UpdateTaskRequestSchema.safeParse({ completed: true, admin: true })
        .success,
    ).toBe(false);
    expect(
      CreateTaskRequestSchema.safeParse({ title: "t", userId: "someone-else" })
        .success,
    ).toBe(false);
  });

  it("rejects invalid field values just like create", () => {
    expect(UpdateTaskRequestSchema.safeParse({ title: "" }).success).toBe(
      false,
    );
    expect(UpdateTaskRequestSchema.safeParse({ priority: 5 }).success).toBe(
      false,
    );
    expect(
      UpdateTaskRequestSchema.safeParse({ deadline: "next tuesday" }).success,
    ).toBe(false);
  });
});
