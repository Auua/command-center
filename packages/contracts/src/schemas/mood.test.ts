...
import { CreateMoodCheckinRequestSchema, MoodCheckinSchema, MoodWindowDaysSchema } from './mood';
...
describe('CreateMoodCheckinRequestSchema', () => {
  ...
  it('rejects an empty or oversized note', () => {
    expect(CreateMoodCheckinRequestSchema.safeParse({ score: 3, note: '   ' }).success).toBe(false);
    expect(
      CreateMoodCheckinRequestSchema.safeParse({ score: 3, note: 'x'.repeat(1001) }).success,
    ).toBe(false);
  });
});
