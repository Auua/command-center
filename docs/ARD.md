...
flowchart TB
    ...
    subgraph backend ["Backend host (Render)"]
...
flowchart LR
    ...
    subgraph domains ["Domain modules"]
        ...
        jp["JapaneseModule<br/>grammar of the day (ADR-012);<br/>WOTD + cards moved to Learning (ADR-024)"]
...
| **MongoDB Atlas**     | journal entries (rich text as structured JSON), braindump notes, learning content (lessons, grammar-of-the-day items)                                               | Document-shaped, schema-flexible content, full-text search (Atlas Search) for journal        |
...
MongoDB collections (owner module in parens): `journal_entries` (Journal), `braindump_notes` (Braindump), `lesson_content` (Learning), `jp_content` (Japanese). All documents carry `userId` and are filtered on it in every query via a repository base class. Learning-center data (word pool, saved cards, per-kind progress, Anki sync state) is deliberately **not** in either database — it lives as files in the private GitHub learning repo (ADR-024).
...
| Secrets           | platform env vars (Vercel/Render/Supabase dashboards); nothing in the repo; `.env.example` documents shape                                                  |
...
| NFR-13 | Portability     | No hard Vercel/Render lock-in: Next.js standalone build + API Dockerfile both runnable anywhere                            | continuous                                                                                        |
...
| **006** | Hosting: Vercel (web) + Render (api + worker) + managed data tiers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Matches README (Vercel); NestJS long-running processes don't fit Vercel functions — worker needs a real process                                                                                                                                                                                                                                                                                      | Everything on Vercel (no persistent worker); Railway/Fly.io (comparable PaaS — Render chosen); a VPS (ops burden)                                                                                                                                                                                                                                                                                                                                                         |
...
ADRs 019–038 are tracked in `docs/adr/REVIEW-QUEUE.md`; 020, 021, 022, 023, 024, 026, 027, 028, 029, 033, 036, 037, and 038 are **accepted** (011 too, tracked in the index), 025 and 034 are **rejected**, and 030/031 are **parked** (product-owner walkthroughs, 2026-07-16/-17); the rest are **proposed and pending review** — see the queue for the review
state of each. One ARD edit stays **owed on approval**: R5 (content sourcing) closes with ADR-032 — licensed bulk datasets for
