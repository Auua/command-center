...
| **MongoDB Atlas**     | journal entries (rich text as structured JSON), braindump notes, learning content (tech lessons)                                                                    | Document-shaped, schema-flexible content, full-text search (Atlas Search) for journal        |
...
MongoDB collections (owner module in parens): `journal_entries` (Journal), `braindump_notes` (Braindump), `lesson_content` (Learning). All documents carry `userId` and are filtered on it in every query via a repository base class. Learning-center data (word pool, authored grammar points, saved cards, per-kind progress, Anki sync state) is deliberately **not** in either database — it lives as files in the private GitHub learning repo (ADR-024; `jp_content` was retired unbuilt when grammar moved there, ADR-012).
