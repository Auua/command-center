...
We will build appreciation as a **standalone widget and a standalone backend module** — not a section of Journal. It gets its own registry entry (`id: "appreciation"`), its own folder under `apps/web/widgets/appreciation/`, its own error and suspense boundaries, and its own `AppreciationModule` in the API.
...
- **Daily prompt:** when `showDailyPrompt` is on and no entry exists for the local day, the input's placeholder becomes a rotating prompt ("What made you smile today?"); once an entry exists it reverts to a neutral "Add an appreciation…". Prompts are copy, not data — a static localized list in the frontend, no backend involvement.
...
No PATCH in v1: entries are one-liners; edit = delete + re-add. Revisit if dogfooding disagrees. Standard per-user throttling applies (§5.2); no route-specific tightening — this is a low-risk write surface.
...
- **Ships as:** one migration (`appreciation_entries` + RLS policies), one API module, one contracts schema pair, one widget folder + registry entry, one default-layout tweak. No changes to Journal, Mood, or the shell.
...
- The prompt list living in frontend copy means changing prompts is a deploy, not a data edit — acceptable for v1.
...
- **Backend-served daily prompts (`GET /appreciation/prompt`).** Rejected for v1: prompts are static copy with no per-user state; an endpoint adds latency and surface for zero benefit. Revisit if prompts become adaptive.
