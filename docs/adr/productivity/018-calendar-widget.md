...
The README scopes a calendar with daily/weekly/monthly views; the ADR places `CalendarModule` among the domain modules (§4.1), lists `calendar_events` as a user-owned Postgres table (§4.3/4.4), and schedules the work for Phase 4 (§9). The dashboard mock does not include a calendar card, so this ADR defines the widget from the ADR and README, keeping the mock's visual language (card-head with kicker, ghost settings button, quick actions in the chrome).
...
- **Timezones and all-day events.** The classic bug: an all-day event stored as a midnight timestamp shifts to the previous day when viewed from a different timezone. NFR targets a travelling single user (see also ADR Q1), so this must be designed away, not patched.
