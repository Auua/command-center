...
flowchart TB
    ...
    learnrepo["GitHub learning-center repo<br/>(Obsidian vault: typed notes · .cc index ·<br/>sync state, ADR-024/040)"]
...
| GitHub (learning repo)    | Vault index, note front-matter progress, Anki sync trigger | API serves cached pool (stale word; cold boot mid-outage → "try again later"); saves error with visible retry; an Actions outage delays sync, loses nothing (ADR-024/026) |
...
MongoDB collections (owner module in parens): `journal_entries` (Journal), `braindump_notes` (Braindump). All documents carry `userId` and are filtered on it in every query via a repository base class. Learning-center data (the Obsidian vault's typed notes and their `status`/`confidence`/`reviewed` front-matter, the vault-generated `.cc/index`, tech lessons, Anki sync state) is deliberately **not** in either database — it lives as files in the private GitHub learning repo, which is the product owner's Obsidian vault (ADR-024 as amended by ADR-040; `jp_content` and `lesson_content` were retired unbuilt when grammar and tech lessons moved there, ADR-012/013 — **Mongo's learning tenancy is zero**).
...
**Anki integration (ADR-024/026):** the backend never talks to Anki, and neither does the browser. every vault note carries a `## Kortit` card (ADR-040); the learning repo's GitHub Action parses them and runs the official `anki` library against AnkiWeb — sync down, upsert notes keyed on the vault's sha1 UID (guid = UID, note type `Japani (Obsidian)`), sync up (**never full-upload** — a CI runner must never overwrite mobile review history), then commit results and deck stats to `sync/state.json`, which the API reads for the status surface. AnkiWeb credentials live only in the learning repo's Actions secrets, never on our hosts.
...
Full ADRs live in `docs/adr/`, grouped into domain subfolders (foundation, productivity, reflection, learning, external-data, lifestyle) with a numbered index at `docs/adr/README.md`; summaries here. ADRs 001–007 were decided with this document (2026-07-11) and written out as full ADRs in `docs/adr/foundation/` on 2026-07-19.
...
| **014** | Streaks widget (accepted 2026-07-18): event-subscribed `StreaksService`, 03:00-local grace boundary for every source (learning kinds pace on UTC but credit home-tz days), `recomputeDay` retraction for declared un-do events (`habit.unmarked`, ADR-027's owed amendment), acknowledge-only learning sources (`wotd.acknowledged`/`grammar.studied`/`lesson.completed`), read-only API — **built 2026-09-18 (Phase 3C):** rollover computed on read (ADR-005/039 deferred the worker); listeners write under the emitting request's JWT via an out-of-band `EventContext`, not the service-role carve-out                                                                                                                                        | Emitters stay streak-unaware via event→streak map; `streak_days` gives idempotent day-marks; one uniform streak-day semantics across sources; retraction keeps undo honest without a general rewrite path; no "at risk" nudges (wellbeing over engagement)                                                                                                                                           | Generic `activity.recorded` event (couples emitters to streaks); strict-midnight boundary (00:30 activity unfairly breaks streaks); UTC streak days for learning sources (splits `local_date` semantics per key); accepting the unmark drift (a visibly un-earned day)                                                                                                                                                                                                    |
...
| **032** | Learning content (accepted 2026-07-18): ingest pinned JMdict / JmdictFurigana / Tatoeba release artefacts into the learning repo's pool (no runtime API); typed `license` block required per item, source × release attribution rows in the pool manifest; JLPT levels are curated annotations ("≈ N4", tanos.co.uk-seeded, chip only when present); grammar/tech/system-design content authored (`proprietary-own`); attribution in the about panel — the deployment and repo are private, so no distribution occurs — **amended by ADR-040 (2026-09-18):** R5 closed by ownership for Japanese; JMdict ingest and attribution withdrawn                                                                                                          | Closes R5 — a pinned ingest has no upstream to be unavailable; no official JLPT lists and no open-licensed grammar dataset exist; EDRDG's "each screen display" clause binds public dictionary displays, recorded as a tripwire: a public deployment owes the WOTD card a persistent footer line on day one, and a public repo carries CC BY-SA                                                      | Jisho's unofficial API (no terms, and its data _is_ JMdict); WaniKani (Tofugu copyright, no redistribution licence); a live dictionary API on the read path; a worker cron over GitHub releases (unattended writes to the app's most important data); LLM-generated grammar (no provenance; quality _is_ the product); always-visible card footer (withdrawn at acceptance — private deployment)                                                                          |
...
| **040** | Obsidian vault as the learning store (accepted 2026-09-18, every decision as drafted): the `learning-center` repo is the product owner's Obsidian vault (~14k typed notes, Finnish-first); `LearningModule` reads a vault-side generated index (`.cc/index/*.jsonl` from front-matter, emitted by a vault Action) and writes progress back into the vault's own `status`/`confidence`/`reviewed` fields; `itemId` = vault path; day pin in `.cc/state.json`; cards are the notes' `## Kortit` sections synced by `sr_to_anki.py`'s parser; "Add to Anki" removed for vault kinds; ADR-024's `pool/`, `progress/`, `cards/` layout superseded, its store decision stands                                                                            | The content, the progress fields, and the cards already exist in the vault at 5× the planned pool and are edited daily in Obsidian; a second layout would leave the vault's review views empty and double every card in Anki; the vault-side indexer keeps ADR-024's small-file cache read path intact at 14k files                                                                                  | Ingest the vault into ADR-024's layout (a copy nobody reads, two progress models); `progress/*.json` mirrored to front-matter (double write); per-file Contents reads or tarball per refresh (rate budget / repo weight); indexer as a monorepo composite action (vault schema and its parser split across repos); progress in the note body; per-note `anki: true` opt-in                                                                                                |
