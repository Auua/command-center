...
describe('formatDueLabel', () => {
  ...
  it('uses the weekday name when within the next 6 days', () => {
    expect(formatDueLabel('2026-07-13', now).label).toBe(weekdayName(new Date(2026, 6, 13)));
    // The 6-day boundary (Thursday 16 July) still gets a weekday name.
    expect(formatDueLabel('2026-07-16', now).label).toBe(weekdayName(new Date(2026, 6, 16)));
  });
