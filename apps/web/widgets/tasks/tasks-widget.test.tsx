...
describe("TasksWidget", () => {
  it("renders open tasks with their priority pill", async () => {
    ...
    expect(
      await screen.findByText("Review ARD feedback notes"),
    ).toBeInTheDocument();
  ...
  it("marks completed tasks checked, struck through, and 'done'", async () => {
    ...
    expect(
      screen.getByText("Review ARD feedback notes").closest("li"),
    ).toHaveClass("done");
  });
  ...
  it("completes an open task via its toggle", async () => {
    ...
    updateMock.mockResolvedValue(
      task({ completedAt: "2026-07-10T10:00:00.000Z" }),
    );
    const user = userEvent.setup();
  ...
  it("reopens a completed task via its toggle", async () => {
    ...
    renderWithQuery(<TasksWidget />);
    await user.click(
      await screen.findByRole("checkbox", { name: /mark .* incomplete/i }),
    );
  ...
  it("deletes a task via its delete button", async () => {
    ...
    renderWithQuery(<TasksWidget />);
    await user.click(
      await screen.findByRole("button", { name: /delete task/i }),
    );
