...
describe('BraindumpWidget', () => {
  ...
  it('shows an empty state when there are no notes', async () => {
    ...
    expect(await screen.findByText(/dump your first thought/i)).toBeInTheDocument();
  });
  ...
  it('shows an error state when loading fails (widget failure posture)', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/couldn.t load braindump/i);
  });
  ...
  it('creates a note on Enter and clears the textarea', async () => {
    ...
    await waitFor(() => expect(createMock).toHaveBeenCalledWith('new idea', expect.anything()));
  ...
  it('deletes a note via its delete button', async () => {
    ...
    await waitFor(() =>
      expect(deleteMock).toHaveBeenCalledWith('665f1e1e1e1e1e1e1e1e1e1e', expect.anything()),
    );
  });
