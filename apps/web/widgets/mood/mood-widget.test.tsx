...
describe('MoodWidget', () => {
  ...
  it('undoes the just-logged check-in', async () => {
    ...
    await waitFor(() => expect(screen.queryByText(/logged low/i)).not.toBeInTheDocument());
  });
  ...
  it('shows an error state when logging fails', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/couldn.t save that check-in/i);
  });
  ...
  it('shows an error state when loading fails', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent(/couldn.t load mood check-ins/i);
  });
});
