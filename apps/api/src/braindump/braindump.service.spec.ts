...
describe('BraindumpService', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [BraindumpService, { provide: BraindumpRepository, useValue: repository }],
    }).compile();
  ...
  it("creates a note scoped to the token's user id", async () => {
    ...
    expect(repository.createForUser).toHaveBeenCalledWith(user.id, note.content);
  });
  ...
  it('updates a note and returns the new contract shape', async () => {
    ...
    expect(repository.updateContentForUser).toHaveBeenCalledWith(user.id, note.id, 'updated');
  });
  ...
  it('deletes a note owned by the user', async () => {
    ...
    await expect(service.deleteNote(user, makeNote().id)).resolves.toBeUndefined();
    expect(repository.deleteForUser).toHaveBeenCalledWith(user.id, makeNote().id);
  });

  it('404s on delete when nothing owned by the user matched', async () => {
    repository.deleteForUser.mockResolvedValue(false);

    await expect(service.deleteNote(user, makeNote().id)).rejects.toBeInstanceOf(NotFoundException);
  });
});
