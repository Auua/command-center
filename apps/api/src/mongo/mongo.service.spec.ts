...
describe('MongoService', () => {
  ...
  it('uses the database named in the connection string', () => {
    const service = new MongoService(makeConfig('mongodb://localhost:27017/custom_db'));
  ...
  it('hands out collections by name', () => {
    ...
    expect(service.collection('braindump_notes').collectionName).toBe('braindump_notes');
  });
});
